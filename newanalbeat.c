...
5 december 2015: add polarity and proportion to AnalyzeBeat
*****************************************************************************/
#include "bdac.h"
#include <stdio.h>
#include <stdlib.h>
...
void AnalyzeBeat(int *beat, int *onset, int *offset, int *isoLevel,
                 int *beatBegin, int *beatEnd, int *amp, int*polarity,
                 int*prop)
{
  ...
  */
  ...
  // changed rapr: maxVI, minVI
  int maxVI=*onset, minVI=*onset;
  ...
  *amp = maxV-minV ;
  ...
  // new 5 december 2015: rapr
  // Compute polarity of QRS and proportions of sections above and below
  // isolevel
  prop[0]=0;
  prop[1]=0;
  prop[2]=0;
  if(maxV-*isoLevel<3.0/8*(*isoLevel-minV)){
    *polarity=2;
    for(i = *onset; i < minVI; ++i)
      if(beat[i]-*isoLevel > prop[0])
        prop[0] = beat[i]-*isoLevel ;
    prop[1] = *isoLevel - minV;
    for(i = minVI+1; i < *offset; ++i)
      if(beat[i]-*isoLevel > prop[2])
        prop[2] = beat[i]-*isoLevel ;
  }
  else if(*isoLevel-minV<3.0/8*(maxV-*isoLevel)){
    *polarity=1;
    for(i = *onset; i < maxVI; ++i)
      if(*isoLevel-beat[i] > prop[0])
        prop[0] = *isoLevel-beat[i] ;
    prop[1]= maxV-*isoLevel;
    for(i = maxVI+1; i < *offset; ++i)
      if(*isoLevel-beat[i] > prop[2])
        prop[2] = *isoLevel - beat[i] ;
  }
  else if(maxVI<minVI){
    *polarity=3;
    for(i = *onset; i < maxVI; ++i)
