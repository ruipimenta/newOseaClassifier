...
int Classify(int *newBeat,int rr, int noiseLevel, int *beatMatch, int *fidAdj,
	...
	AnalyzeBeat(newBeat, &onset, &offset, &isoLevel,
                    &beatBegin, &beatEnd, &amp, &polarity, prop) ;
	...
	if((beatClass == UNKNOWN) && (morphType < MAXTYPES))
          {
            // change rapr 23/12/205
            // remove rules that classify beats as normal to avoid
            // classifying PVCs as normal
...
int TempClass(int rhythmClass, int morphType,
	...
	if(domType < 0)
        ...
        // 5 Dez 2015, rapr: new Rule: use polarity //

        float threshold=0.5;
        int polarityMorph, propMorph[3], polarityDom, propDom[3];
        polarityMorph=GetBeatPolarity(morphType);
        GetBeatProp(morphType,propMorph);
        polarityDom=GetBeatPolarity(domType);
        GetBeatProp(domType,propDom);
        if( (domType>-1) && (polarityDom!=polarityMorph) ){
          if( (polarityDom==1) && (polarityMorph==2) )
            return(PVC) ;
          if( (polarityDom==1) && (polarityMorph==3) ){
            if(propDom[2]<1)
              return(PVC) ;
            else if(1.0*propDom[1]/propDom[2]-
                    1.0*propMorph[1]/propMorph[2]>threshold)
              return(PVC) ;
          }
         if( (polarityDom==1) && (polarityMorph==4) ){
           if(propDom[0]<1)
              return(PVC) ;
           else if(1.0*propDom[1]/propDom[0]-
                    1.0*propMorph[1]/propMorph[2]>threshold)
              return(PVC) ;
         }
          if( (polarityDom==2) && (polarityMorph==1) )
            return(PVC) ;
          if( (polarityDom==2) && (polarityMorph==3) ){
          // result seems to be always PVC (????)
            if(propDom[0]<1)
              return(PVC) ;
            else
              if( 1.0*propDom[1]/propDom[0]-
                  1.0*propMorph[2]/propMorph[1]>threshold)
                return(PVC) ;
          }
          if( (polarityDom==2) && (polarityMorph==4) ){
            if(propDom[2]<1)
              return(PVC) ;
            else if(1.0*propDom[1]/propDom[2]-
                    1.0*propMorph[1]/propMorph[2]>threshold)
          ...
          if( (polarityDom==3) && (polarityMorph==1) ){
            if(propDom[2]<1)
              return(PVC) ;
            if(abs(1.0*propDom[1]/propDom[2]-
                   1.0*propMorph[1]/propMorph[2])>threshold)
              return(PVC) ;
          }
         if( (polarityDom==3) && (polarityMorph==2) ){
            if(abs(1.0*propDom[1]/propDom[2]-
                   1.0*propMorph[0]/propMorph[2])>threshold)
              return(PVC) ;
          }
          if( (polarityDom==3) && (polarityMorph==4) ){
            if(abs(1.0*propDom[1]/propDom[2]-
                   1.0*propMorph[0]/propMorph[1])>threshold)
              return(PVC) ;
          }
         if( (polarityDom==4) && (polarityMorph==1) ){
           if(abs(1.0*propDom[1]/propDom[2]-
                  1.0*propMorph[0]/propMorph[1])>threshold)
             return(PVC) ;
         }
         if( (polarityDom==4) && (polarityMorph==2) ){
           if(propDom[2]<1)
              return(PVC) ;
           if(abs(1.0*propDom[1]/propDom[2]-
                  1.0*propMorph[1]/propMorph[2])>threshold)
         ...
         if( (polarityDom==4) && (polarityMorph==3) ){
           if(abs(1.0*propDom[1]/propDom[2]-
                  1.0*propMorph[0]/propMorph[1])>threshold)
             return(PVC) ;
         }
