...
some level of encapsulation:
	...
	GetBeatEnd -- Returns the ending point for a given beat type.
        new 5 Dez 2015 rapr:
        GetBeatPolarity --- Returns the QRS polarity for a given beat type.
        GetBeatProp --- Returns the QRS proportions for a given beat type.
...
// new rapr 5 Dez 2015: QRS polarity, prop -> see newanal.c
int BeatPolarity[MAXTYPES] ;
int BeatProp[MAXTYPES][3]; // prop is a 3d vector
...
/******************************************************
	GetBeatPolarity returns the QRS polarity for a
	given type.
******************************************************/

int GetBeatPolarity(int type)
	{
	return(BeatPolarity[type]) ;
	}

/******************************************************
	GetBeatProp returns the QRS proportions for a
	given type.
******************************************************/

void GetBeatProp(int type, int*prop)
	{
          int i;
          for( i=0; i<3;++i)
            prop[i]=BeatProp[type][i];
	}
...
int NewBeatType(int *newBeat )
{
  ...
  if(TypeCount < MAXTYPES)
    {
      ...
      BeatPolarity[TypeCount] = polarity; 
      for(i = 0; i < 3; ++i)
        BeatProp[TypeCount][i]=prop[i];
  ...
  // that has occurred the fewest number of times.
  ...
  else
    {
      ...
      BeatAmps[mcType] = amp ;
      BeatPolarity[mcType] = polarity; 
      for(i = 0; i < 3; ++i)
        BeatProp[mcType][i]=prop[i];
      return(mcType) ;
    }
}
...
void UpdateBeatType(int matchType,int *newBeat, double mi2,
	...
	BeatPolarity[matchType] = polarity;
        for(i = 0; i < 3; ++i)
        BeatProp[matchType][i]=prop[i];
...
int GetDominantType(void)
	...
	int type, totalCount ;
        int min4dominant=60; // was 300 (magic number)
...
void BeatCopy(int srcBeat, int destBeat)
	...
	BeatAmps[destBeat] = BeatAmps[srcBeat] ;
        BeatPolarity[destBeat] = BeatPolarity[srcBeat] ; 
        for(i = 0; i < 3; ++i)
          BeatProp[destBeat][i]=BeatProp[srcBeat][i];
