...
#define BEAT_QUE_LENGTH	10	      	// Length of que for beats awaiting						// classification.  Because of
...
/******************************************************************************
	ResetjustClassify() resets static variables required for beat detection and
	classification.
*******************************************************************************/
...
****************************************************************************/
int justClassify(int ecgSample, int *beatType, int *beatMatch,int *QRSlocations,
                 int numQRSloc)
{
  ...
  // change rapr Dez 2015
  static ordQRS=0; // order of the next QRS location
  static int sampleCount=0;
  sampleCount++; // this sample
  ...
  detectDelay=(BEATLGTH-FIDMARK)*(SAMPLE_RATE/BEAT_SAMPLE_RATE);

  if(sampleCount<QRSlocations[ordQRS]+detectDelay){
    // Return if no beat is ready for classification.
    NoiseCheck(ecgSample,0,rr, beatBegin, beatEnd) ; // here value of rr
                                                     // does not matter
    return 0;
  }
  ...
  if(ordQRS>0)
    rr=QRSlocations[ordQRS]-QRSlocations[ordQRS-1];
  else
    rr=sampleCount;
  ...
  return detectDelay ; // rapr was (detectDelay-fidAdj)
}
